...
4 -> nrcpt
*/
func TestAssertQmgrMatches(t *testing.T) {
	line := "Feb 10 10:30:43 hostname postfix/qmgr[2318978]: D2B38E21CF: from=<test@myDomain.test.io>, size=4435, nrcpt=1 (queue active)"
	matches := qmgrInsertLine.FindStringSubmatch(line)
	assert.Equal(t, "test@myDomain.test.io", matches[1])
	assert.Equal(t, "myDomain.test.io", matches[2])
	assert.Equal(t, "4435", matches[3])
	assert.Equal(t, "1", matches[4])
}
